...
def gower_matrix(data_x, data_y=None, weight=None, is_cat_features=None):
    """calculate gower dist between data_x and data_y"""
    # function checks
    X = data_x
    if data_y is None:
        Y = data_x
    ...
    if is_cat_features is None:
        if not isinstance(X, np.ndarray):
            is_number = np.vectorize(lambda x: not np.issubdtype(x, np.number))
            is_cat_features = is_number(X.dtypes)
        else:
            is_cat_features = np.zeros(x_n_cols, dtype=bool)
            for col in range(x_n_cols):
                if not np.issubdtype(type(X[0, col]), np.number):
                    is_cat_features[col] = True
    else:
        is_cat_features = np.array(is_cat_features)

    # print(is_cat_features)
    ...
    Z_num = Z[:, np.logical_not(is_cat_features)]
    ...
    weight_cat = weight[is_cat_features]
    weight_num = weight[np.logical_not(is_cat_features)]
...
def gower_topn(data_x, data_y=None, weight=None, is_cat_features=None, n=5):

    if data_x.shape[0] >= 2:
        raise TypeError("Only support `data_x` of 1 row. ")
    dm = gower_matrix(data_x, data_y, weight, is_cat_features)
